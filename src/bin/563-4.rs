...
// Helper function to convert a vector with placeholders to a vector with `Option<i32>`
fn to_option_vec(values: Vec<i32>, placeholder: i32) -> Vec<Option<i32>> {
    values
        .into_iter()
        .map(|x| if x == placeholder { None } else { Some(x) })
        .collect()
}

fn main() {
    let values = to_option_vec(vec![4, 2, 9, 3, 5, -1, 7], -1);
