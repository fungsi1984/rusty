...
impl Solution {
    ...
    // Function that takes clean input and converts it to Vec<Option<i32>>
    pub fn from_clean_input(input: Vec<&str>) -> Vec<Option<i32>> {
        input
            .into_iter()
            .map(|x| match x {
                "null" => None,
...
fn main() {
    // Clean input without needing Some() or None; just integers and "null"
    let input = vec!["5", "4", "9", "1", "10", "null", "7"];

    // Convert clean input into Vec<Option<i32>>
    let input_as_vec = Solution::from_clean_input(input);
    ...
    // Add code here to print or validate the modified tree
}
