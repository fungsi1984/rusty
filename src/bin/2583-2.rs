...
// New function to convert a vector of Option<i32> into a TreeNode structure
    ...
    while i < values.len() {
        if let Some(val) = queue.pop_front() {
            if i < values.len() && values[i].is_some() {
                let left_node = Rc::new(RefCell::new(TreeNode::new(values[i].unwrap())));
            ...
            if i < values.len() && values[i].is_some() {
                let right_node = Rc::new(RefCell::new(TreeNode::new(values[i].unwrap())));
