...
impl Solution {
    pub fn valid_arrangement(pairs: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
        ...
        for pair in &pairs {
            ...
            if !in_out_degree.contains_key(&u) {
                in_out_degree.insert(u, 0);
            }
            if !in_out_degree.contains_key(&v) {
                in_out_degree.insert(v, 0);
            }
        }
        ...
        for pair in &pairs {
            let u = pair[0];
            let v = pair[1];
            *in_out_degree.get_mut(&u).unwrap() += 1; // out-degree
            *in_out_degree.get_mut(&v).unwrap() -= 1; // in-degree
        }
